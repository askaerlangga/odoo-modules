...
| [**X2many Search Bar**](./ae_x2many_search) | `ae_x2many_search` | Adds an Odoo-style client-side search bar with a Group By menu above `one2many` / `many2many` tree views. Supports field-scoped filtering and grouping shown as insertion-ordered facets, collapse/expand per group, and seamless Section & Note support when `account` is installed. | 17.0.2.0.0 |
